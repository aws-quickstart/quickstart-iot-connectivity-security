...
* IoT device security management enabling nabling AWS IoT Device Defender ML Detect and Audit
...
=== Device Management 
Before registration, devices are placed in a preset group called ‘Inactive group’ with IoT policies not allowing communication between the device and AWS IoT Core. 
Upon registration and activation by the end user, devices are automatically placed under another preset group called ‘Active group’ with IoT policies granting 
