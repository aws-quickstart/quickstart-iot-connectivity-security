...
* AWS IoT Device Management, which organizes, monitors, and manages IoT things (devices registered in the AWS IoT registry). This service assigns devices to thing groups based on lifecycle stage—inactive, active, quarantine, and troubleshooting—and attaches IoT policies to each group.
* AWS IoT Device Defender, which audits cloud-side device configurations against AWS security best practices. This service monitors device behaviors and responds to anomalies by using built-in mitigation actions. For example, it might move an IoT thing from the active group to the quarantine group. You can configure Device Defender to publish metrics and send you alarms.
