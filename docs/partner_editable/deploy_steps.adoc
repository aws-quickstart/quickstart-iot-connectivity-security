...
.. Parameter: CACertificateID, see instructions for creating a https://docs.aws.amazon.com/iot/latest/developerguide/create-your-CA-cert.html[certificate authority^] and registering within https://docs.aws.amazon.com/iot/latest/developerguide/register-CA-cert.html[AWS IoT^]
.. Device/client certificate: Use the CA to create certificates for devices. The *Common Name* of the device certificate you create should also be the serial number you assign to the device and provide to end users for device activation. 
.. For details on each parameter, see the link:#_parameter_reference[Parameter reference] section of this guide. When you finish reviewing and customizing the parameters, choose *Next*.
